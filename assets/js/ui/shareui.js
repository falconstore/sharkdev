...
export class ShareUI {
  ...
  async share(calculator) {
    console.log('📤 Compartilhando:', calculator);
    try {
      let data, url;
  ...
  getArbiProData() {
    ...
    if (!app) throw new Error('ArbiPro não encontrado');
    return {
  ...
  getFreeProData() {
    ...
    if (!iframe?.contentDocument) throw new Error('FreePro não encontrado');
